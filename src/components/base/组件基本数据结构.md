#### <span id="public">公共属性</span>
|序号|名称|类型|描述|来源|
|----|----|----|----|----|
|1|define|props|组件定义信息载体，类型为object|上层组件传递|
|2|focusId|props|与label标签配合使用，优化用户体验用(需要后台支持)|上层组件传递，值暂定为上层组件的uid值|
|3|formTmp|props|弹出页临时表单元素所依附的表单对象在vuex中的key值|上层表单传递，暂定为对应table的uid值|
|4|form|computed|表单元素所依附的表单对象在vuex中的key值|为formTmp或form(默认值)|
|5|readonly|computed|表单元素是否只读|单独请求获取，从vuex中获取|
|6|required|computed|表单元素是否必填|从define对象中获取|
|7|visible|computed|表单元素是否可见|从define对象中获取(这个值是否有必要，数据存放在vuex中，不需要使用隐藏表单)|
|8|name|computed|表单元素的name值|从define中获取，暂时没有name属性，值为组件的uid值|
|9|dataType|computed|表单元素的基本数据类型|从define中获取，暂时没有dataType属性，默认返回String|
|10|reset|computed|是否重置表单内元素的值|vuex维护|
|11|hasError|computed|标志位: 输入内容是否符合校验要求|组件和vuex维护|
|12|objectModel|computed|组件需要绑定的值|值从vuex中获取|
|13|isRelated|computed|标志位: 表单元素是否与其他元素关联|从define中获取|
|14|boundTarget|如果isRelated为真，则为对应的对象的值的数组|通过define的值从vuex中获取|
|15|errorMessage|输入不符合校验规则时的错误提示|同步校验为组件自己维护，异步校验可能需要后台支持|


#### Date
[公共属性](#public)见上表

|序号|名称|描述|来源|
|----|----|----|----|
|16|type|显示类型。可选值为date，daterange，datetime，datetimerange，year，month|define中获取，默认为date。但是否需要所有实现|
|17|format|返回的日期数据字符串表现形式|define中获取，默认为'yyyy-MM-dd'|
|18|placeholder|无输入时的提示文字|从define中获取，默认为'请选择日期'|

#### Decimal
[公共属性](#public)见上表

|序号|名称|描述|来源|
|----|----|----|----|
|16|placeholder|无输入时的提示文字|从define中获取，默认为'请输入一个小数'|
|17|icon|错误或只读时输入框后面的提示|根据hasError或readonly来判断，也可为后台控制|
|18|minValue|允许输入的最小值|从define中获取，默认为Number.MAX_SAFE_INTEGER|
|19|maxValue|允许输入的最大值|从define中获取，默认为Number.MIN_SAFE_INTEGER|
|20|precision|数值精度|从define中获取，默认为10|

#### Input
[公共属性](#public)见上表

|序号|名称|描述|来源|
|----|----|----|----|
|16|maxLength|允许输入的字符串的最大长度|从define中获取，默认值为Number.MAX_SAFE_INTEGER|
|17|minLength|允许输入的字符串的最小长度|从define中获取，默认值为0|
|18|pattern|对输入字符串进行校验的正则表达式字符串表示|从define中获取，默认值为'.*'|
|19|placeholder|没有输入时的提示信息|从define中获取，默认值为'请输入相关信息'|
|20|icon|错误或只读时输入框后面的提示|根据hasError或readonly来判断，也可为后台控制|

#### Label(该组件尚未使用)

|序号|名称|描述|来源|
|----|----|----|----|
|1|define|组件定义信息载体，类型为object|上层组件传递|
|2|label|表单组件左侧的标签提示|从define中获取，默认为'标签'|
|3|visible|该组件是否可见|从define中获取，默认为true|
|4|bindTarget|与label标签相关联的输入组件的key值|从define中获取，默认为''|

#### LabelInput

|序号|名称|描述|来源|
|----|----|----|----|
|1|define|组件定义信息载体，类型为object|上层组件传递|
|2|uid|组件的uid，作为label标签和表单元素关联的key值|目前由上级组件传递，但应该从define中获取|
|3|label|label标签的内容|从define中获取，过程比较复杂|
|4|hidden|label标签和与之关联的表单元素是否显示|默认为false，目前未被使用|
|5|ui_define|组件定义对象|与define重复|
|6|itemType|标识表单元素的具体形式|从define中获取，默认值为'mInput'|
|7|labelWidth|label标签的长度|从define中获取，默认值为120|

#### Loading

|序号|名称|类型|描述|来源|
|----|----|----|----|----|
|1|label|props|显示loading动画时的提示|默认为'Loading...'|
|2|show|props|标志位: loading动画是否显示|默认为false|