...
export default {
    ...
    data () {
        return {
            ...
            // 过渡效果
            transition: d3.transition().duration(2000).ease(d3.easeCircleInOut),
            // 堆栈图group
    ...
    computed: {
        ...
        yScaleBar () {
            return d3.scaleLinear()
                .domain([0, d3.max(this.stackData[this.stackData.length - 1], d => d[1])])
    ...
    methods: {
        ...
        drawAxis () {
            ...
            if (currAxisBar.empty()) {
                this.svg.append('g')
                    ...
                    .attr('transform', `translate(${this.percentPaddingLeft}, ${this.percentPaddingTop})`)
                    .call(this.yAxisBar)
            } else {
                currAxisBar.attr('class', 'axis yAxisBar')
                    .attr('transform', `translate(${this.percentPaddingLeft}, ${this.percentPaddingTop})`)
            }
            ...
            if (currAxisLine.empty()) {
                this.svg.append('g')
                    ...
                    .attr('transform', `translate(${this.width - this.percentPaddingRight}, ${this.percentPaddingTop})`)
                    .call(this.yAxisLine)
            } else {
                currAxisLine.attr('class', 'axis yAxisLine')
                    .attr('transform', `translate(${this.width - this.percentPaddingRight}, ${this.percentPaddingTop})`)
            }
            ...
            if (currXaxis.empty()) {
                this.svg.append('g')
                    ...
                    .attr('transform', `translate(${this.percentPaddingLeft}, ${this.height - this.percentPaddingBottom})`)
                    .call(this.xAxis)
            } else {
                currXaxis.attr('transform', `translate(${this.percentPaddingLeft}, ${this.height - this.percentPaddingBottom})`)
            }
        },
        // 绘制堆栈图group
        drawStackGroup () {
            // update部分 
            this.stackGroupsUpdate = this.svg.selectAll('g.bar-container')
                .data(this.stackData)
            
            // enter部分
            this.stackGroupsEnter = this.stackGroupsUpdate.enter()
                .append('g')
        ...
        drawStackBar () {
            ...
            // enter部分
            this.stackGroupsEnter.selectAll('rect.bar')
                .data(d => d)
                .enter()
                .append('rect')
                .attr('class', 'bar')
                ...
                .attr('y', d => this.height - this.percentPaddingBottom - this.yScaleBar(d[1]))
        ...
        drawLengend () {
            ...
            // 绘制图例图标
            let stackLabelRects = this.stackGroupsEnter.append('rect')
            ...
            // 绘制说明文字
            this.stackGroupsEnter.append('text')
        ...
        drawLine () {
            ...
            this.chartLine = this.lineGroup.append('path')
            ...
            this.chartLine.attr('stroke-dasharray', `0, ${length}`)
                .transition()
                .duration(2500)
                .attr('stroke-dasharray', `${length}, 0`)

            // 折线文字数据
            this.lineGroup.selectAll('text .line-text')
                .data(this.chartData)
                .enter()
                .append('text')
                .attr('class', 'line-text')
                .attr('x', d => this.xScale(d[this.xAxisKey]) + this.percentPaddingLeft + this.xScale.bandwidth() / 2 + 20)
                .attr('y', d => this.height - this.percentPaddingBottom - this.yScaleLine(d.line))
        ...
        stackMouseoutByLegend () {
            this.stackGroupsEnter.transition()
                .duration(500)
                .style('fill', (d, i) => this.colors[i])
        },
        lineMouseover () {
            this.chartLine.transition()
                .duration(500)
                .attr('stroke-width', 5)
        },
        lineMouseout () {
        ...
        stackMouseover (parentNode, self, data) {
            ...
            this.svg.selectAll('.bar-container')
            ...
            this.svg.selectAll('.bar-container')
                .each(function (data, index) {
                    ...
                    d3.select(this)
                        ...
                        .filter((d, i) => i === currIndex)
                        .transition(vue.transition)
                        .attr('y', d => vue.height - vue.percentPaddingBottom - vue.yScaleBar(d[1]))
