    ...
    $.ajax({
	    ...
	    success: function (data) {
	    	if (data.result == 'success') {
	    		...
	    		execTransferList = proWf.execTransferList
        ...
        canvas.width = 0.91 * ww
        canvas.height = 0.84 * hh
...
function resetStatusLoc (dx, dy) {
    ...
    for (var i = 0; i < flowLinkList.length; i++) {
        ...
        if (link1.formJobMetaId == selJobMeta.jobId) {
            var pt0 = ptList[0]
            pt0.x += dx
            pt0.y += dy
        }
        if (link1.toJobMetaId == selJobMeta.jobId) {
            var pt = ptList[ptList.length - 1]
            pt.x += dx
            pt.y += dy
        }
...
function resetStatusMap (jobMeta) {
    ...
    if (proWf.swimlaneList.length > 0) {
        var ww1 = 0
        var swimlane
...
function adjustmentDy4Transfer (link, selPt, y) {
    ...
    if (idx == 0) {
        ...
        for (var i = 0; i < proWf.proJobList.length; i++) {
            jobMeta = proWf.proJobList[i]
            if (jobMeta.jobId == link.formJobMetaId) {
        ...
        for (var i = 0; i < proWf.proJobList.length; i++) {
            ...
            if (jobMeta.jobId == link.toJobMetaId) {
                var middleY = jobMeta.sy + (jobMeta.hh / 2)
                if (adjustmentGap > Math.abs(middleY - y)) {
                    return middleY
                }
...
function adjustmentDx4Transfer (jobLink, selPt, x) {
    ...
    if (idx == 0) {
        ...
        for (var i = 0; i < proWf.proJobList.length; i++) {
            ...
            if (jobMeta.jobId == jobLink.formJobMetaId) {
        ...
        for (var i = 0; i < proWf.proJobList.length; i++) {
            ...
            if (statusMap.jobId == jobLink.toJobMetaId) {
...
function setProWfMeta (proWfMeta) {
    ...
    if (proWfMeta != null && proWfMeta.swimlaneList != null) {
        var swimlane
...
function drawChart () {
    ...
    for (var i = 0; i < jobList.length; i++) {
		...
		// if (okStatusList.indexOf(jobMeta) >= 0) {
...
function drawNodeContent (gc, jobMeta) {
    ...
    if (PRO_JB_MT_CLS_WORK_FLOW == jobMeta.jobClass) {
        ...
        var inputIcon = inputIconMap[jobMeta.jobId]
        var inputVal = inputValueMap[jobMeta.jobId]

        var outputIcon = outputIconMap[jobMeta.jobId]
        var outputVal = outputValueMap[jobMeta.jobId]

        if (selJobMeta == jobMeta) {
            inputVal = inputFullValueMap[jobMeta.jobId]
            outputVal = outputFullValueMap[jobMeta.jobId]
        }
...
function drawLink (gc, link) {
    if (execTransferList.indexOf(link) >= 0) {
        gc.strokeStyle = DARKORANGE_COLOR
...
function findSelObj (x, y) {
    ...
    if (proWf.swimlaneList.length > 0 && SWIMLANE_HH > y) {
        var x1 = sx
        var swimlane
        for (var i = 0; i < proWf.swimlaneList.length; i++) {
            swimlane = proWf.swimlaneList[i]
    ...
    for (var i = 0; i < flowLinkList.length; i++) {
        ...
        for (var j = 0; j <= ptMaxIdx; j++) {
            pt1 = ptList[j]
            if (Math.abs(pt1.x - x + sx) <= 8 &&
				Math.abs(pt1.y - y + sy) <= 8) {
                selPt = pt1
                if (pt1.joinNodeId > 0) {
                    var stepRect
                    for (var k = 0; k < flowNodeList.length; k++) {
                        ...
                        if (stepRect.jobId == pt1.joinNodeId) {
                            joinNode = stepRect
                            break
                        }
                    }
...
function findSelStep4DrawLink (x, y) {
    ...
    lp.joinNodeId = selJobMeta.jobId
    return lp
}
...
function setNewPointInJoinPt (x, y) {
    ...
    for (var i = 0; i < nodeList.length; i++) {
        if (selPt.joinNodeId == nodeList[i].jobId) {
...
function delSelNode () {
    ...
    for (var i = linkList.length - 1; i >= 0; i--) {
        link1 = linkList[i]
        if (link1.formJobMetaId == selJobMeta.jobId ||
				link1.formJobMetaId == selJobMeta.jobId) {
            linkList.splice(i, 1)
        }
    }
    var nodeList = proWf.proJobList
...
function showAuthorityGroupMember (me) {
    ...
    if (proWf.swimlaneList.length > 0 && swimlaneDragMode == 0 && SWIMLANE_HH > y) {
        var x1 = sx
        var swimlane
        for (var i = 0; i < proWf.swimlaneList.length; i++) {
            swimlane = proWf.swimlaneList[i]
	...
	if (checkNotEmpty(jobMeta.inputFormDataExpress)) {
		...
		for(String item : inputVals){
			...
			if(items[0].equalsIgnoreCase("FORM")){
				...
				if (null != formMeta) {
					if (0 == idx) {
						inputIconMap.put(jobMeta.jobId, jobMeta.iconName);
						if (1 < inputVals.length) {
							inputValueMap.put(jobMeta.jobId, jobMeta.name + " ...");
						} else {
							inputValueMap.put(jobMeta.jobId, jobMeta.name);
						}
						inputFullValueMap.put(jobMeta.jobId, jobMeta.name);
					} else {
						inputFullValueMap.put(jobMeta.jobId, inputFullValueMap.get(jobMeta.jobId) + " " + jobMeta.name);
					}
					idx++;

				}else{
					inputIconMap.put(jobMeta.jobId, null);
					inputValueMap.put(jobMeta.jobId, null);
					inputFullValueMap.put(jobMeta.jobId, null);
				}
			...
			if (items[0].equalsIgnoreCase("NEW")) {
				...
				if (null != formMeta) {
					if (0 == idx) {
						...
						outputIconMap.put(jobMeta.jobId, formMeta.iconName);
						if (1 < outputVals.length) {
							outputValueMap.put(jobMeta.jobId, "[+" + strTmp + "] " + formMeta.name + " ...");
						} else {
							outputValueMap.put(jobMeta.jobId, "[+" + strTmp + "] " + formMeta.name);
						}
						outputFullValueMap.put(jobMeta.jobId, "[+" + strTmp + "] " + formMeta.name);
					} else {
						String val = outputFullValueMap.get(jobMeta.jobId);
						outputFullValueMap.put(jobMeta.jobId, val + " " + formMeta.name);
					}
					idx++;

				} else {
					outputIconMap.put(jobMeta.jobId, null);
					outputValueMap.put(jobMeta.jobId, null);
					outputFullValueMap.put(jobMeta.jobId, null);
				}
			}
