...
function drawBizChart (items) {
	// 获取画面画布
    canvas = document.getElementById('BizChartCanvas')
...
function drawChart (pos) {
    ...
    if (items.items.length > 0) {
        ...
        var shapes = items.items

        for (var i = 0; i < shapes.length; i++) {
            var points = shapes[i].points
            var firstPoint = shapes[i].points[0]
            var curPoint = null
            ...
            for (var j = 1; j < points.length; j++) {
                curPoint = points[j]
                if (j < points.length) {
                    gc.lineTo(curPoint.x, curPoint.y)
                } else {
            ...
            if (shapes[i].status == 'used') {
                gc.fillStyle = 'rgb(246, 196, 145)'
            } else if (shapes[i].status == 'empty') {
                gc.fillStyle = 'rgb(222, 222, 220)'
            } else if (shapes[i].status == 'noRent') {
                gc.fillStyle = 'rgb(227, 85, 107)'
            }
            gc.fill()
            gc.stroke()
            ...
            if (shapes[i].status == 'used') {
            	name = shapes[i].company.name
                if (name.match(reg).length * 14 > shapes[i].weight) {
                    name = shapes[i].company.shortName
                };
            } else if (shapes[i].status == 'empty') {
                name = '空置'
            } else if (shapes[i].status == 'noRent') {
            ...
            gc.fillText(name,
                firstPoint.x + shapes[i].weight / 2,
                firstPoint.y + shapes[i].height / 2,
                firstPoint.x + shapes[i].weight - 4)

            gc.textAlign = 'left'
            gc.textBaseline = 'top'

            gc.fillText(shapes[i].name + ' 面积：' + shapes[i].area,
                firstPoint.x + 2,
                firstPoint.y + 2,
                firstPoint.x + shapes[i].weight - 4)

            if (pos && gc.isPointInPath(pos.x, pos.y)) {
                // 如果传入了事件坐标，就用isPointInPath判断一下
                // console.log(shapes[i].name);

                $('#bizChartInfo').find('.modal-body').html(template('bizChartTmpl', shapes[i].company))
                $('#bizChartInfo').modal('show')
            }
        }
    }
...
function setSelFilter () {
    ...
    var options = {
        theme: 'blue', // 选项：default;blue;green。 默认灰色
    ...
    $('#platTreeNav').listFilter(options)
}

// 获取用户名
function getUserName () {
    $.get('/getUserName', function (result) {
        $('#username').text('欢迎你，' + result.username)
        if (result.sysIcon) {
            $('.logo img').attr('src', result.sysIcon)
        } else {
            $('.logo img').attr('src', 'img/toptimus-logo.png')
        }
    })
}
